...
/* ================= 内部全屏浏览器与手机自带返回拦截 ================= */
window.openLinkInDefaultBrowser = function(url, e) {
    if (e && e.stopPropagation) e.stopPropagation();
    if (e && e.preventDefault) e.preventDefault();
    ...
    const modal = document.getElementById('internalBrowserModal');
    const frame = document.getElementById('internalBrowserFrame');
    if (modal && frame) {
        frame.src = url;
        modal.style.display = 'flex';
        // 向浏览器 history 压入一个状态，使得手机自带返回键先关闭此模态，而不是退出应用
        history.pushState({ modal: 'internalBrowser' }, '');
    } else {
        window.open(url, '_blank');
    }
};

window.closeInternalBrowser = function() {
    const modal = document.getElementById('internalBrowserModal');
    const frame = document.getElementById('internalBrowserFrame');
    if (modal) {
...
window.addEventListener('popstate', function(e) {
    ...
    const emojiNamer = document.getElementById('emojiNamerIframeContainer') || document.getElementById('emojiNamerBuilderPanel');
    if (emojiNamer && emojiNamer.style.display !== 'none' && !emojiNamer.classList.contains('hidden')) {
        if (typeof closeEmojiNamerModal === 'function') closeEmojiNamerModal();
        if (typeof closeEmojiNamer === 'function') closeEmojiNamer();
        return;
    }
